...
| 1 | **帧头 2** | 1 | `0x5A` |
| 2 | 舵机 1 ID | 1 | |
| 3 | 舵机 1 位置_L | 1 | Pos (0-4095) |
| 4 | 舵机 1 位置_H | 1 | |
| 5 | 舵机 1 速度_L | 1 | Speed (0-3000, 0=Max) |
| 6 | 舵机 1 速度_H | 1 | |
| 7 | 舵机 1 加速度 | 1 | Acc (0-254, 0=Max) |
| ... | ... | ... | (共 6 个舵机) |
| 32 | 舵机 6 ID | 1 | |
| 33 | 舵机 6 位置_L | 1 | |
| 34 | 舵机 6 位置_H | 1 | |
...
| 36 | 舵机 6 速度_H | 1 | |
| 37 | 舵机 6 加速度 | 1 | |
| 38 | **校验和** | 1 | 前 38 字节累加和的低 8 位 |

### 2.3 数据示例
控制 ID=1, Pos=2048, Speed=1000, Acc=50:
`A5 5A 01 00 08 E8 03 32 ... [其余5个舵机全0] ... [Checksum]`

### 2.4 校验算法 (Python)
```python
# len(packet) == 38 (Header + 6*6)
...
*   `Core/`, `Drivers/`, `MDK-ARM/`: STM32 下位机固件源码及工程。
*   `nano_code/`: 上位机调试代码。
    *   `servo_ctrl.py`: Jetson Nano / Linux 环境调试脚本。
    *   `servo_ctrl_win.py`: Windows 环境调试脚本。
*   `GEMINI.md`: 项目技术文档（本文档）。
